...
def copy_public_paquet(idpaquet, idcreateur):
    ...
    print(f"{bddGen.COLOR['green']}{func_name()}{bddGen.COLOR['end']}")
...
def update_revision(idutilisateur, idcarte, idboite,rang):
    sql = "UPDATE revision SET rang=%s, date_derniere_revision=CURDATE() WHERE idboite=%s and idutilisateur=%s AND idcarte=%s;"
    param = (rang,idboite, idutilisateur, idcarte)
    return bddGen.updateData(func_name(), sql, param, None)
...
def get_nb_cartes_maitrisees(iduser):
    sql="Select COUNT(Distinct(r.idcarte)) as nb_cartes_maitrisees from revision as r join boite as b on b.idboite=r.idboite where b.rang=%s and r.idutilisateur=%s"
    param=(0,iduser)
    return bddGen.selectOneData(func_name(),sql,param, None)
